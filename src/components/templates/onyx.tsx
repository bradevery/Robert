...
// Picture component for profile images
const Picture = ({ className }: { className?: string }) => {
  ...
  const fontSize = useArtboardStore(
    (state) => state.resume.metadata?.typography?.font?.size || 14
  );
...
// BrandIcon component for social media icons
const BrandIcon = React.forwardRef<HTMLImageElement, { slug: string }>(
  ({ slug }, ref) => {
    if (slug === 'linkedin') {
      return (
        <img
...
const Header = () => {
  ...
  return (
    <div className='flex items-center justify-between pb-5 space-x-4 border-b border-primary'>
      <Picture />
      ...
      {profiles.visible && profiles.items.length > 0 && (
        <div
          className='grid text-right gap-x-4 gap-y-1'
    ...
    <section id={section.id} className='grid'>
      ...
      >
        {section.items
          ...
          .map((item: any) => {
            const url = (urlKey && get(item, urlKey as string)) as
              | URL
              | undefined;
            const level = (levelKey && get(item, levelKey as string, 0)) as
              | number
              | undefined;
...
const Certifications = () => {
  const section = useArtboardStore(
    (state) => state.resume.sections.certifications
  );
...
const Publications = () => {
  const section = useArtboardStore(
    (state) => state.resume.sections.publications
  );
...
export const Onyx = ({ columns, isFirstPage = false }: TemplateProps) => {
  // Protection contre les colonnes undefined
  if (!columns || !Array.isArray(columns) || columns.length < 2) {
    console.warn('Onyx template: Invalid columns provided', {
      columns,
      type: typeof columns,
